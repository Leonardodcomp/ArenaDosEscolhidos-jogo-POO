# ArenaDosEscolhidos-jogo-POO
Jogo criado para compor parte da nota da disciplina de Programação Orientada a Objetos


Observações iniciais:

- Para integrar um RPG usual, estamos usando a biblioteca java.util.Random para usar o método random.nextInt(), esse terá a função de dado, ajudando nos calculos de probabilidade


Prompts de IA utilizados:

- 

Integrantes(5): Leonardo Souza Silva, Alan Cristian de Jesus Dias, Gabriel de Sena Barbosa, Jefersandro Vieira Silva, Joao Pedro Andrade Sa Gonzaga
